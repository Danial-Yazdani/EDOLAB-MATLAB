...
function [Optimizer, Problem] = IterativeComponents_DPCPSO(Optimizer, Problem)
    ...
    for ii = 1 : Optimizer.SwarmNumber-1
        for jj = ii+1 : Optimizer.SwarmNumber
            ...
            if distance < Optimizer.ExclusionRadius
    ...
    for ii = 1 : Optimizer.SwarmNumber
        if Optimizer.pop(ii).IsExcluded == 1
            ...
            Optimizer.pop(ii).Velocity = -4 + (4 - (-4)) * rand(size(Optimizer.pop(ii).X));  % Initialize particle velocities
    ...
    if converge_count == Optimizer.SwarmNumber
        ...
        Optimizer.pop(worst_idx).Velocity = -4 + (4 - (-4)) * rand(size(Optimizer.pop(worst_idx).X));  % Initialize particle velocities
...
function mergedPop = mergeSubPopulations(pop1, pop2)
    ...
    [mergedPop.GbestValue, index] = max(mergedPop.PbestValue);
    mergedPop.GbestPosition = mergedPop.PbestPosition(index, :);
    mergedPop.IsConverged = 0;
    mergedPop.IsStagnated = 0;
    mergedPop.IsExcluded = 0;
    mergedPop.localIter = min(pop1.localIter, pop2.localIter);
end
