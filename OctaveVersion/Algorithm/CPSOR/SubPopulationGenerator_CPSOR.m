...
%**************************************************************************************************
function [Swarm , Problem] = SubPopulationGenerator_CPSOR(Dimension,MinCoordinate,MaxCoordinate,InitSwarm,maxSize,Problem)
...
population = struct('X',[],'Gbest_past_environment',[],'Velocity',[],'Shifts',[],'FitnessValue',[],'PbestPosition',[],'IsConverged',[],'PbestValue',[],'GbestValue',[],'GbestID',[],'GbestPosition',[],'Center',[],'InitRadius',[],'CurrentRadius',[]);
...
for i=1:length(m_group)
    ...
    Swarm(i).Velocity = -(MaxCoordinate-MinCoordinate)/4 + (2 * (MaxCoordinate-MinCoordinate)/4) *rand(size(m_group(i).X,1),Dimension);    
    Swarm(i).Shifts = [];
