...
%**************************************************************************************************
function [Optimizer, Problem] = IterativeComponents_DynPopDE(Optimizer,Problem)
...
if(Optimizer.Iteration <= 2)
    ...
    for ii=1 : Optimizer.SwarmNumber
        if(Optimizer.pop(ii).ReInitState == 1)
            [Optimizer.pop(ii),Problem] = SubPopulationGenerator_DynPopDE(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,1,Optimizer.IndivSize,Problem,Optimizer.DiversityFlag,Optimizer.RandomFlag,Optimizer.StrategyFlag);
        end
...
elseif(Optimizer.Iteration > 2)
    ...
    if sum(abs(Optimizer.CurFitness - Optimizer.PreFitness)) == 0
        [Optimizer.pop(end+1),Problem] = SubPopulationGenerator_DynPopDE(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,1,Optimizer.IndivSize,Problem,Optimizer.DiversityFlag,Optimizer.RandomFlag,Optimizer.StrategyFlag);
        Optimizer.SwarmNumber = Optimizer.SwarmNumber + 1;
        Optimizer.CurFitness(end+1,1) = Optimizer.pop(end).BestValue;
        Optimizer.PreFitness(end+1,1) = 0;
        if Problem.RecentChange == 1
           return;
        end
    end
            ...
            if(Optimizer.pop(ii).ReInitState == 0 && Optimizer.pop(jj).ReInitState == 0 && dis < Optimizer.ExclusionLimit)
                ...
                if MidFitness > Optimizer.pop(ii).BestValue && MidFitness > Optimizer.pop(jj).BestValue
                    if(Optimizer.pop(ii).BestValue> Optimizer.pop(jj).BestValue)
                        Optimizer.PreFitness(jj,1) = Optimizer.CurFitness(jj,1);
                        Optimizer.CurFitness(jj,1) = Optimizer.pop(jj).BestValue;
                        if (Optimizer.CurFitness(jj,1) - Optimizer.PreFitness(jj,1)) == 0
                            Optimizer.pop(jj).ReInitState = 1;
                        else
                            Optimizer.pop(jj).DeleteState = 1;
                        end
                    else
                        Optimizer.PreFitness(ii,1) = Optimizer.CurFitness(ii,1);
                        Optimizer.CurFitness(ii,1) = Optimizer.pop(ii).BestValue;
                        if (Optimizer.CurFitness(ii,1) - Optimizer.PreFitness(ii,1)) == 0
                            Optimizer.pop(ii).ReInitState = 1;
                        else
                            Optimizer.pop(ii).DeleteState = 1;
                        end
                    end
                    if Problem.RecentChange == 1
                       return;
    ...
    while(1)
        for ii=1 : Optimizer.SwarmNumber
            if(Optimizer.pop(ii).DeleteState == 1)
                Optimizer.pop(ii) = [];
                Optimizer.PreFitness(ii) = [];
                Optimizer.CurFitness(ii) = [];
                Optimizer.Performance(ii) = [];
                Optimizer.SwarmNumber = Optimizer.SwarmNumber- 1;
                break;
            end
        end
        if(ii == Optimizer.SwarmNumber)
            break;
        end
    end
    Optimizer.ExclusionLimit = (Optimizer.MaxCoordinate - Optimizer.MinCoordinate)/(2*Optimizer.SwarmNumber.^(1/Optimizer.Dimension));
    ...
    for ii=1 : Optimizer.SwarmNumber
        ...
        Optimizer.PreFitness(ii,1) = Optimizer.CurFitness(ii,1);
        Optimizer.CurFitness(ii,1) = Optimizer.pop(ii).BestValue;
        if abs(Optimizer.CurFitness(ii,1) - Optimizer.PreFitness(ii,1)) > 0
            Optimizer.pop(ii).Pen = 0;
        else
            Optimizer.pop(ii).Pen = Optimizer.pop(ii).Pen + 1;
        end
    end
end
end
...
%     for ii = EvolvedID
    for ii = 1:Optimizer.SwarmNumber
        if(Optimizer.pop(ii).Pen > 0)
            Performance(ii) = (1+abs(Optimizer.pop(ii).BestValue - Optimizer.pop(Optimizer.WorstPopID).BestValue))*(1+abs(Optimizer.CurFitness(ii,1) - Optimizer.PreFitness(ii,1)))/Optimizer.pop(ii).Pen;
        else
