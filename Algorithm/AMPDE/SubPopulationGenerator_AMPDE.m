...
%**************************************************************************************************
function [Swarm,RemainIndivs,Problem] = SubPopulationGenerator_AMPDE(Dimension,MinCoordinate,MaxCoordinate,InitSwarm,Problem)
RemainIndivs = [];
...
while(1)
    ...
    for i=1:length(m_group)
        if(size(m_group(i).X,1) < 5)
            for r = 1:size(m_group(i).X,1)
                RemainIndivs(end+1,:) = m_group(i).X(r,:);
            end
            m_group(i) = [];
            break;
        end
...
population = struct('X',[],'Donor',[],'Trial',[],'Gbest_past_environment',[],'FitnessValue',[],'IsHibernated',[],'StagnatingCount',[],'IsStagnated',[],'BestValue',[],'BestPosition',[],'GbestID',[],'Center',[],'InitRadius',[],'CurrentRadius',[]);
