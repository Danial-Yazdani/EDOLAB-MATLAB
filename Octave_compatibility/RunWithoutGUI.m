...
%Necessary packages to load for Octave versios
pkg load statistics;
pkg load io;
...
AlgorithmsList = cell(length(AlgorithmsFloder)-2, 1);  % Compatible with Octave: Use cell array instead of string array
for i = 3:length(AlgorithmsFloder)
    AlgorithmsList{i-2,1} = AlgorithmsFloder(i).name; % Compatible with Octave: Use curly braces for cell array
end
BenchmarksFloder = dir([projectPath,'\Benchmark']);
BenchmarksList = cell(length(BenchmarksFloder)-5, 1);  % Compatible with Octave: Initialize as a cell array
BenchmarksCount = 0;
for i = 3:length(BenchmarksFloder)
    if(isempty(strfind(BenchmarksFloder(i).name,'.m')))
        BenchmarksCount = BenchmarksCount + 1;
        BenchmarksList{BenchmarksCount,1} = BenchmarksFloder(i).name; %Compatible with Octave: Use curly braces for cell array
    end
end
